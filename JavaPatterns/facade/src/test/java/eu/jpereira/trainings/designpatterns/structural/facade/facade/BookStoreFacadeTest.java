...
public class BookStoreFacadeTest extends AbstractClientTest {
	...
	protected BookstoreFacade createFacade( ) {
		...
		DefaultBookStoreFacade bookStore = new DefaultBookStoreFacade();
		bookStore.setWharehouseService(warehouseService);
		bookStore.setCustomerService(customerNotificationService);
		bookStore.setCustomerDBService(customerService);
		bookStore.setBookDBService(bookService);
		bookStore.OrderingService(orderingService);
		return bookStore;
	}
}
